...
pub fn parse_gitmodules(repo_path: &Path) -> Result<Vec<SubmoduleInfo>> {
    ...
    let output = Command::new("git")
        .args(["config", "-f", ".gitmodules", "--get-regexp", r"^submodule\..*\.(path|url)$"])
    ...
    if !output.status.success() {
        ...
        bail!("Failed to parse .gitmodules: {}", String::from_utf8_lossy(&output.stderr));
    }
    ...
    for line in stdout.lines() {
        ...
        if key_parts.len() < 3 { continue; }
        
        let name = key_parts[1].to_string();
        let prop = key_parts[2];
        
        let entry = submodules.entry(name.clone()).or_insert_with(|| (String::new(), String::new()));
        if prop == "path" {
    ...
    let result = submodules.into_iter()
...
pub fn check_submodule_status(repo_path: &Path, submodule_path: &Path) -> Result<(bool, VcsStatus, Option<String>, Option<String>)> {
